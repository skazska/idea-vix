...
// Helper functions
async function createTestBoard(page: Page): Promise<string> {
  await page.goto('/board');
  
  // Create a test board using existing patterns
  const boardName = `workshop-test-board-${Date.now()}`;
  ...
  await form.locator('textarea[name="description"]').fill('Test board for workshop E2E tests');
  await form.locator('button[type="submit"]').click();
  
  // Wait for modal to close
  await expect(page.getByTestId('modal-centered')).toBeHidden();
  
  // Find the created board in the list and open it
  const boardList = page.getByRole('list');
  const boardRow = boardList.getByRole('listitem').filter({ hasText: boardName });
  await expect(boardRow).toBeVisible();
  
  // Click to open board detail
  await boardRow.getByRole('link').click();
  
  // Wait for navigation to board detail page
...
function getWorkshopLocators(page: Page): WorkshopLocators {
  const section = page.getByTestId('board-section-workshop');
  const manager = page.getByTestId('workshop-manager');
  
  return {
    section,
    toggle: page.getByTestId('board-section-workshop-toggle'),
    manager,
...
// Tests
test.describe('@Smoke Board Workshop Items Management', () => {
  ...
  test('should persist workshop items after page reload', async ({ page }) => {
    const workshopLocators = getWorkshopLocators(page);
    
    await openWorkshopSection(workshopLocators);
    
    // Create a shape
    const shapeName = `Persistent Shape ${Date.now()}`;
    const itemSlug = await createWorkshopItem(page, workshopLocators, 'shapes', {
      name: shapeName,
    ...
    // Re-open workshop section
    const newWorkshopLocators = getWorkshopLocators(page);
    await openWorkshopSection(newWorkshopLocators);
    
    // Verify item still exists
    await expect(page.getByTestId(`workshop-item-${itemSlug}`)).toBeVisible();
    await expect(page.getByTestId(`workshop-item-${itemSlug}`)).toContainText(shapeName);
    
    // Verify tab count is correct
    await expect(newWorkshopLocators.shapesTab).toContainText('1');
  });
});
